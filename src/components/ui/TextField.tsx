...
const TextField: React.FC<Props> = (props) => {
  ...
  return (
    <div>
      ...
      <input
        className={`block w-full ${classStyle ? classStyle : "rounded-md"} border border-gray-200 bg-zinc-50/30 px-3 py-1.5 text-base outline-none transition-colors placeholder:text-center focus:border-[#ffb9e2] lg:text-lg`}
        style={{ direction }}
        id={name}
        type={type}
        value={field?.value}
