            ...
            {filteredFeatures.length === 0 && query !== '' ? (
              ...
              filteredFeatures.map((size) => (
                ...
                >
                  {({ selected, active }) => (
                    <>
                      <span className={`block truncate ${selected ? 'font-medium' : 'font-normal'}`}>{size.name}</span>
                      {selected ? (
                        <span
                          className={`absolute inset-y-0 left-0 flex items-center pl-3 ${
                            active ? 'text-[#e90089]' : 'text-[#e90089]'
...
//         ...prev.filter((v) => !stateColorData.some((c) => c.id === v.id)),
//         ...stateColorData,
//       ])
//       if (features) {
//         const updatedFeatures: ProductFeature = {
...
//     if (stateFeatureValueData) {
//       setSelectedFeatureValues((prev) => [
//         ...prev.filter((v) => !stateFeatureValueData.some((f) => f.id === v.id)),
//         ...stateFeatureValueData,
//       ])
//       if (features) {
//         const updatedFeatures: ProductFeature = {
//           ...features,
//           values: stateFeatureValueData,
//         }
//         onFeatureSelect(updatedFeatures)
//       }
//       setStateFeatureValueData!([])
...
//   useEffect(() => {
//     if (stateSizeData) {
//       setSelectedSizes(stateSizeData)
//       onFeatureSelect(stateSizeData)
//     }
//   }, [stateSizeData])
