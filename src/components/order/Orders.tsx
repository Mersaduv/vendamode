...
export default function Orders() {
  ...
  useEffect(() => {
    if (data) {
      const pending = data?.data?.pagination?.data!.filter((item) => item.delivered === false)
      const success = data?.data?.pagination?.data!.filter((item) => item.delivered === true)

      setPendingOrder(pending?.length ?? 0)
      setSuccessOrder(success?.length ?? 0)
    }
  }, [data])
